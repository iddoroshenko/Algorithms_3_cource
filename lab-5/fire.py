...
def single_move(current_solve, a, num_of_cells):
    ...
    for i in range(len(cells_details)):
        if i != old_cell:
            ...
            if new_f >= best_f:
                best_cells_machine = new_cells_machine
                best_cells_details = cells_details
                best_f = new_f
...
def get_ans(a):
    ...
    for i in range(1, len(similarity)):
        if similarity[i][0] > delta:
            if similarity[i][1] not in cells_details[0]:
                cells_details[0].append(similarity[i][1])
            if similarity[i][2] not in cells_details[0]:
                cells_details[0].append(similarity[i][2])
    for detail in range(n):
        if detail not in cells_details[0]:
            cells_details[1].append(detail)
    cells_machines = machines_cells_selecting(a, cells_details, num_of_cells)

    best_solve = (fitness(cells_machines, cells_details, a, num_of_cells), cells_details, cells_machines)  # S**
    current_best_solve = best_solve  # S*
    current_solve = best_solve  # S
    goto_2 = True
    while True:
        if goto_2:
            T_0 = 1
        ...
        while counter_mc < L and counter_trapped < L / 2:
            ...
            #if counter % D == 0:
            #    cells_solve = exchange_move(current_solve)
        ...
        if T <= T_f or counter_stagnant > 100:
            if current_best_solve[0] > best_solve[0]:
                ...
                num_of_cells += 1
                goto_2 = True
                current_solve[1].append([])
                current_solve[2].append([])
            else:
                break
        else:
            T = T * alpha
